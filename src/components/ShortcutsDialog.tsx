...
const SHORTCUTS: { keys: string[]; label: string }[] = [
  ...
  { keys: ["1", "–", "8"], label: "Списки задач (на экране Задачи)" },
            ...
            <div key={s.label} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                {s.keys.map((k) => (
                  <kbd
                    key={k}
                    className="flex h-6 min-w-6 items-center justify-center rounded border border-border bg-secondary px-1.5 text-xs font-medium"
                  >
                    {k}
                  </kbd>
                ))}
